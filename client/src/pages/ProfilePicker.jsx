...
export default function ProfilePicker() {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                const res = await axiosInstance.get(`/users/patients/${authUser._id}`); // server endpoint below
                if (!mounted) return;
                setRels(res.data || []);
            } catch (err) {
    ...
    // If no relationships, still allow continue (no doctor selected)
    const handleSelect = (doctorId) => {
        setActiveDoctor(doctorId || null);
        navigate("/home"); // or wherever your app's main page is
    };
